...
class DT1DAdapter(nn.Module):
    def __init__(
        ...
        **legacy,
        ...
        # Backward-compatible translation from the pre-v0.7 DT1D API.
    ...
    def _build_weighted_dt1d_kernel_1d(
        ...
        """Build shifted symmetric DT1D axial kernels.

        The learnable sequence f[-M],...,f[M] is symmetric and group-shared:
            f[0]=alpha_0, f[+m]=f[-m]=alpha_m.

        For a 1D signal g[n], the shifted symmetric DT1D branch is
            y[n] = 1/2 sum_m f[m] (
        ...
        # This preserves the shifted-symmetric DT1D pattern while preventing uncontrolled
        # amplification on finite feature maps.
        denom = wg.abs().sum(dim=1, keepdim=True).clamp_min(1e-6)
        wg = wg / denom
        return self._expand_group_kernel_to_channels(wg)

    # Compatibility alias for pre-v0.7 code. New code uses the canonical DT1D name.
        ...
        if self.scale_adaptive:
            ...
            for ai, axis_name in enumerate(self.axis_names):
                for si, dilation in enumerate(self.dilations):
                    w1d = self._build_weighted_dt1d_kernel_1d(ai, si, x.device, x.dtype)
