#!/usr/bin/env python3
"""Preflight checks for WHC-DT1D with static/global axis-scale gates.

Run before long Kaggle experiments:
    PYTHONPATH=. python tools/preflight_whc_dt1d_static.py --device auto
"""
...
_SPEC = importlib.util.spec_from_file_location("hcc_adapter_direct", _ROOT / "models" / "hcc_adapter.py")
...
def make_adapter(C=16, M=1, no_pw=False) -> HCCAdapter:
    return HCCAdapter(
        C=C,
        M=M,
        h=1,
...
def test_kernel_and_routing(device: torch.device) -> None:
    ...
    k = m._build_weighted_hcc_kernel_1d(0, 0, device, torch.float32)
    expected_K = 2 * m.M + 3
...
def test_torchvision_integration(device: torch.device) -> None:
    ...
    for mod in list(m.modules()):
        ...
        if ch and not hasattr(mod, "pet_adapter"):
            ad = HCCAdapter(C=ch, M=1, dilations="1,2,4", scale_adaptive=True, axis="hw", no_pw=True, gate_init=0.01)
...
def main() -> None:
    ...
    print("[ALL OK] WHC-DT1D static-gate preflight passed")
