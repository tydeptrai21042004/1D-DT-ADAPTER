...
DT1D=load(ROOT/'models'/'dt1d_adapter.py','dt1d_validation')
...
def main():
        ...
        for shape in shapes:
         for dtype in dtypes:
          for no_pw in no_pws:
            ...
            old=build(DT1D,m,dils,pad,dtype,no_pw,False)
            new=build(DT1D,m,dils,pad,dtype,no_pw,True)
            with torch.no_grad():
    ...
    old=build(DT1D,1,(1,2,4),'reflect',torch.float32,False,False)
    new=build(DT1D,1,(1,2,4),'reflect',torch.float32,False,True)
    with torch.no_grad():
        old.alpha.normal_(); old.axis_scale_logits.normal_(); old.gate.fill_(0.31)
        for p in old.pw.parameters(): p.normal_(0,0.1)
    new.load_state_dict(old.state_dict(),strict=True)
    ...
    old_train=build(DT1D,1,(1,2,4),'reflect',torch.float32,False,False)
    new_train=build(DT1D,1,(1,2,4),'reflect',torch.float32,False,True)
    with torch.no_grad():
    ...
    out=ROOT/'logs'/'reference'/'benchmarks'/'validation_dt1d_math_realization_regenerated.json'; out.parent.mkdir(parents=True, exist_ok=True); out.write_text(json.dumps(result,indent=2)); print(json.dumps(result,indent=2))
