...
def test_weighted_hcc_kernel_length_and_l1():
...
def test_axis_scale_weights_static_shape_sum():
    m = HCCAdapter(C=16, M=1, dilations="1,2,4", scale_adaptive=True, axis="hw")
...
def test_forward_backward():
    torch.manual_seed(0)
    m = HCCAdapter(C=8, M=1, dilations="1,2", scale_adaptive=True, axis="hw", no_pw=False, gate_init=0.01)
...
def test_single_dilation_mode():
    m = HCCAdapter(C=8, M=1, h=1, dilations=None, scale_adaptive=False, axis="hw", no_pw=True)
