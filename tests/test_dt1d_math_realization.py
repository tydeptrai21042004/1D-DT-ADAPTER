...
# The branch-form evaluator is retained in the same source file behind
# exact_cost_realization=False.  Using it as the reference makes this test
# repository-portable and avoids the previous hard-coded /mnt/data dependency.
DT1D = load_class(ROOT / "models" / "dt1d_adapter.py", "dt1d_math_realization")


def make_reference(**common):
    return DT1D(
        **common,
...
def test_no_parameter_or_checkpoint_change():
    original = make_reference(**kwargs())
    optimized = make_optimized(**kwargs())
...
def test_exact_mapping_and_gradients_against_branch_reference(
    m, dilations, padding, shape
):
    common = kwargs(m=m, dilations=dilations, padding=padding, c=7, no_pw=False)
    original = make_reference(**common)
    optimized = make_optimized(**common)
...
def test_dynamic_program_matches_global_partition_optimum():
    for m in (0, 1, 2, 3):
        for dilations in (
            ...
            model = make_optimized(**kwargs(m=m, dilations=dilations, padding="zeros"))
...
def test_fifty_step_adamw_trajectory_and_metrics_are_preserved():
    ...
    original = make_reference(**kwargs(c=8, no_pw=False))
    optimized = make_optimized(**kwargs(c=8, no_pw=False))
...
def test_only_runtime_graph_change_is_axial_call_partition():
    ...
    before = make_reference(**common)
    after = make_optimized(**common)
    ...
    for name, model in (('before', before), ('after', after)):
        ...
        original_build = model._build_weighted_dt1d_kernel_1d
        ...
        model._build_weighted_dt1d_kernel_1d = counted_build
        model(x)
        counts[name] = count
